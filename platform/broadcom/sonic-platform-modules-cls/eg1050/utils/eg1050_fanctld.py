...
class Eg1050FanControl(daemon_base.DaemonBase):
    global MODULE_NAME
    global SENSOR_PARAM
    ...
    def __init__(self, log_level, fan_count):
        ...
        super(Eg1050FanControl, self).__init__(MODULE_NAME)
        ...
        try:
            ...
            self.log_error("Failed to init Eg1050FanControl due to {}, so exiting...".format(repr(e)))
            sys.exit(1)
    ...
    def thermal_shutdown(self, reason):
        cmd = ['/usr/local/bin/eg1050_platform_shutdown.sh', reason]
...
def main(argv):
    ...
    fanctl = Eg1050FanControl(log_level, fan_count)

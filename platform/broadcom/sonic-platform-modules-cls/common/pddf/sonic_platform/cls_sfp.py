...
SFP_RESET_FILE_PATH = "/tmp/SFP{}_reset_ts.txt"
...
class ClsPddfSfp(PddfSfp):
    ...
    def is_in_docker(self):
        # Check for /.dockerenv or cgroup docker marker
        if not self.__docker:
            self.__docker = os.path.exists("/.dockerenv") or any("docker" in line for line in open("/proc/1/cgroup"))
    ...
    def write_ts(self, file_path):
        ...
        if self.is_in_docker():
            ...
            # Running on host: use docker exec to write inside docker
            cmd = ["docker", "exec", "pmon",\
                  "bash", "-c", f"echo '{ts}' > {file_path}"]
            subprocess.run(cmd)
    ...
    def read_ts(self, file_path):
        ...
        if self.is_in_docker():
            ...
            # Read timestamp from file inside docker using docker exec
            cmd = ["docker", "exec", "pmon", "cat", file_path]
    ...
    def remove_ts(self, file_path):
        if self.is_in_docker():
            ...
            # Remove file inside docker using docker exec (no shell=True)
            cmd = ["docker", "exec", "pmon",\
                  "rm", "-f", file_path]

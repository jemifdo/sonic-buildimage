...
class Es1010FanControl(daemon_base.DaemonBase):
    global MODULE_NAME
    global SENSOR_PARAM
    ...
    def __init__(self, log_level, fan_count):
        ...
        super(Es1010FanControl, self).__init__(MODULE_NAME)
        ...
        try:
            ...
            self.log_error("Failed to init Es1010FanControl due to {}, so exiting...".format(repr(e)))
            sys.exit(1)
    ...
    def thermal_shutdown(self, reason):
        cmd = ['/usr/local/bin/es1010_platform_shutdown.sh', reason]
...
def main(argv):
    ...
    fanctl = Es1010FanControl(log_level, fan_count)

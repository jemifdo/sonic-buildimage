...
class Chassis(PddfChassis):
    ...
    def __init__(self, pddf_data=None, pddf_plugin_data=None):
        ...
        self.sfp_port_list = list(range(49, 52+1))
        self.global_fan_led = "green"
        self.chassis_fan_dir = self.get_eeprom().vendor_ext_str()
    ...
    def set_global_fan_led_status(self):
        color = "green"
        for fan in self.get_all_fans():
            fan_status = fan.get_fan_led_status()
            if fan_status == "amber":
                color = "amber"
                break

        if color == self.global_fan_led:
            return True

        self.global_fan_led = color
        try:
            if color == "green":
                cmd = ['i2cset', '-y', '-f', '0x1', '0x32', '0x45', '0xce']
            else:
                cmd = ['i2cset', '-y', '-f', '0x1', '0x32', '0x45', '0xcd']
    ...
    def get_global_fan_led_status(self):
        color = "green"
        try:
            cmd = ['i2cget', '-y', '-f', '0x1', '0x32', '0x45']
            p = subprocess.Popen(cmd, stdout=subprocess.PIPE, universal_newlines=True)
            data = p.communicate()
